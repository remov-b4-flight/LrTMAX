 ...
 *	@pre	ENCSW_Stat	current key status
 */
void EmulateMIDI() {
	...
	if (isAnyMoved) {
		...
		}else if( ENCSW_Stat.wd & MASK_ENC ) { //Check encoder's move
			//Send CC message from encoder
